...
function loading(data){
	...
	$("#loader--audioWave").css("z-index", "99999999");
}
/////////加载动画 end////////////////////////////////
/////////显示提示信息 start////////////////////////////////
...
(function($) {
    ...
    $.ajax = function(options) {
        ...
        var _opt = $.extend(options, {
            ...
            beforeSend : function(XHR) {
            	// 提交前回调方法
                $("body").find("button").prop("disabled", true);	//赵滨	关闭button按钮

            	$load$++;
            	loading(false);
            	loading({type:Math.ceil(Math.random()*4)});
            	callback.beforeSend(XHR);
            },
            complete : function(XHR, TS) {
				// 请求完成后回调函数 (请求成功或失败之后均调用)。
                $("body").find("button").prop("disabled", false);	//赵滨	打开button按钮

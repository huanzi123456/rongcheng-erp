...
(function ($) {
    ...
    $.fn.citys = function(parameter,getApi) {
        ...
        var defaults = {
            dataUrl:'/util/region/json/region/list.json',     //数据库地址
...
function getRegionNameByCode(code){
	var province,city,area,parent,present;
	$.ajax({
        url:"/util/region/json/region/list.json",
        type:'GET',
        crossDomain: true,
        dataType:"json",
        jsonpCallback:'jsonp_location',
        ...
        success:function(data){
			 ...
			 if(regionCode){   //如果有地区编码
				 var pc = regionCode - regionCode%1e4;
				 if(data[pc]){
				     province = data[pc];
				     present = data[pc];
				     parent = "中国";
				 ...
				 var cc = regionCode - (regionCode%1e4 ? regionCode%1e2 : regionCode);
				 if(data[cc]){
				     city = data[cc];
				     present = data[cc];
				     parent = data[pc];
				 ...
				 var ac = regionCode%1e2 ? regionCode : 0;
				 if(data[ac]){
				     area = data[ac];
				     present = data[ac];
				     parent = data[cc];
	...
	return {"province":province,"city":city,"area":area,"parent":parent,"present":present};
}